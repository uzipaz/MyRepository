...
int main()
{
	...
	Vertex *Graph = NULL;
	time_t tt;
	double TimeElapsedReadingData = 0.0;
	...
	if (readFromFileData(Graph, size) && readFromFileInfo(start, goal))
	{
		...
		//{
		    TimeElapsedReadingData = difftime(time(NULL), tt);
		    cout << "The total time required to read data from disk is " << TimeElapsedReadingData << " seconds\n";
			cout << "Press Enter to Execute the algorithm\n";
			cin.get();
			clock_t t = clock();
			...
			cout << "\nTotal clocks per seconds are " << CLOCKS_PER_SEC << "\n";
			cout << "\nThe algorithm took " << ((float)t)/CLOCKS_PER_SEC << " seconds\n";
			delete [] path;
		//}
